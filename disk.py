...
def append_log(number, i_d, name, time_out, time_in, total):
    with open('history.txt', 'a') as file:
         file.write('\n'+ str(number) + ', ' + str(i_d) + ', ' + str(name) + ', ' + str(time_out) +', ' + str(time_in) + ', ' + str(total))

def rewrite_checkin(dict_log, number, time_in, total):
    with open('history.txt', 'w') as file:
        new_log = core.log_line(dict_log, number, time_in, total)
...
def check_out(dict_inventory, i_d,name, number, time_out):
    ...
    append_log(number, i_d, name, time_out, 'N/A', 'N/A')
